...
The probability of a collision now depends on the value of our random number $r$. Because $2 \leq r < p$, it holds that $r$ is in the cyclic group $\mathbb{Z}^*_p$. From this, we can define the set $\langle r \rangle \defeq \{r^0, r^1,...,1\}$ -- we know that this group eventually terminates by Lagrange's Theorem, as proven in Theorem 7.14 of \cite{katz:lagrange}. We can define the order of the set $\langle r \rangle$ as the smallest number $i$ such that $r^i = 1$. Katz and Lindell \cite{katz:cyclic-groups} show that if $i$ is the order of $\langle r \rangle$, then $r^x = r^y$ if and only if $x = y \mod i$.
...
\subsection{Investigate Edge Cases}

\subsection{More Thorough Testing of Periodic Patterns}

\subsection{Other Measurements of Interest}
