...
Most implementations of KMP simply consist of an array of $m$ integers to act as a failure table, a string of $m$ characters for the pattern, and a counter of the number of characters that have matched so far, with the counter reaching $m - 1$ representing the accept state. But this is actually a simplification of the algorithm. As pointed out in CLRS[TODO: FIND CHAPTER], the correctness of Knuth-Morris-Pratt is proven because this array, string and integer represent a finite state automata: The array of integers represent the state the automata should fall back to if the pattern does not match, the string shows the next character that needs to arrive in the text in order for the automata to move one state closer to the accept state, and the counter represents the current state the automata is in, with $m - 1$ being the accept state of the automata.

We can show this with an example: Table~\ref{tab:kmp-pattern-failure} shows the pattern and failure table for the pattern $aabab$, and Figure~\ref{fig:kmp-pattern-failure} shows the finite state automata for the same pattern.
...
\begin{figure}[t]
  \centering
    ...
    \end{tikzpicture}
  \caption{Finite state automata for pattern $aabab$}
  \label{fig:kmp-pattern-failure}
\end{figure}

Because the Aho-Corasick algorithm generalises Knuth-Morris-Pratt by taking this finite automata and adapting it for multiple patterns, I decided a suitable way of starting the project was to implement KMP as a finite automata.

This meant an important first decision to make: How to implement the automata? As a finite state automata is based on graphs, there are three main options available:
...
For implementation, I used the C Minimum Perfect Hashing library, with a data structure based on my summer project as mentioned in Section~\ref{sec:summer}. One of these three cases occurs when a structure is searched with key $key$:
...
Because the keys are a single character, each search can be evaluated in constant time.
