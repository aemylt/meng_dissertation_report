...
Instead of storing the entire pattern in a single fingerprint, the pattern was broken up into $\lfloor \log_2 m\rfloor$ fingerprints, each a power of two prefix of the patter. These fingerprints denoted $\phi_i$, are computed as follows:
...
If the above condition does not hold and these two conditions do not match, there is no general consensus on how to handle this case of a non-periodic VO. Breslauer and Galil\cite{Breslauer:2014:RSS:2660854.2635814} recommend not inserting the occurance into the pattern, yet reporting the index as a match against the whole pattern anyway and accepting some chance of false positives. Other papers\cite{JPS:2013} ignore this issue altogether and leave it as a decision for implementation. % TODO: Might want to remove this last sentence if possible.
